...
%add onset delay for higher flowrate:
    ...
    %R/R0:
    R0(CC)=nanmean(ratio1);
    ratio1=(ratio1./R0(CC))*1;
    ratio2=(ratio2./R0(CC))*1;
    ...
    if ~isempty (ImagingData{F}.XY_1)
        ...
        cc=1;
        for i=(sec+1):sec/2:length(speed)-(sec/2)
            ms=nanmean(speed(i-(sec/2):i+(sec/2)));
            nn=numel(find(isnan(speed(i-(sec/2):i+(sec/2)))));            
            if ms<p_cutoff & nn<(sec/2) 
                pauses(i:i+sec/2)=1;
                cc=cc+1;
            end
        end
        ...
        pauses(1:(sec+1))=NaN;pauses(end-(sec/1.9):end)=NaN;
    ...
    if plots==1
        ...
        % plot ratios:
        plot(tv, gcamp1,'k','linewidth',1.5);
        plot(tv2,gcamp2,'k','linewidth',1.5);
