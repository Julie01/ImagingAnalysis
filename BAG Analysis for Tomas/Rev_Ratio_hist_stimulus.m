...
for F=1:length(ImagingData)
    ...
    cherry=medfilt1(ImagingData{F}.cherry2(1:7280),5);
    cherry=[NaN(7280,1); dn; cherry];
    gcamp=medfilt1(ImagingData{F}.gcamp2(1:7280),5);
    gcamp=[NaN(7280,1);dn; gcamp];
  ...
  ratio=(gcamp/nanmean(gcamp));
    ...
    for rev=1:length(RevON)
        ...
        if RevON(rev)>900 & RevON(rev)+400<length(ratio)
            
            prerev(cc,:)=ratio(RevON(rev)-899:RevON(rev)+400);
            cc=cc+1;
            % if reversal is too close to start of trace:
        elseif RevON(rev)<900
            
            prerev(cc,:)=NaN(1,1300);
            prerev(cc,RevON(rev)+1:1300)=ratio(1:1300-RevON(rev));
            cc=cc+1;
            %or to end
        elseif RevON(rev)+400>length(ratio)
            prerev(cc,:)=NaN(1,1300);
            missing=length(ratio)-(RevON(rev)+400);
            prerev(cc,1:1300+missing)=ratio(RevON(rev)-899:end);
            cc=cc+1;
        end
...
hold on
plot([900 900],[min(nanmedian(prerev))-0.02 max(nanmedian(prerev))+0.02],'r')
set(gca,'XTick',[1:100:size(prerev_n,2)])
set(gca,'XTicklabel',round(0:3.33:50))
title([dirname(cd) ': median R reversals stimulus'])
...
title([dirname(cd) ': mean R reversals stimulus'])
xlabel('s')
ylabel('mean R BAG')
