...
cherry=medfilt1(ImagingData{F}.cherry2(1:7280),5);
    cherry=[NaN(7280,1); cherry];
    gcamp=medfilt1(ImagingData{F}.gcamp2(1:7280),5);
    gcamp=[NaN(7280,1); gcamp];
...
title([dirname(cd) ': median R/R0 reversals stimulus episode'])
...
hold on
plot([900 900],[min(nanmean(prerev_n)-0.02) max(nanmean(prerev_n)+0.02)],'r')
set(gca,'XTick',[1:100:size(prerev_n,2)])
set(gca,'XTicklabel',round(0:3.33:50))

title([dirname(cd) ': mean R/R0 reversals stimulus episode '])
xlabel('s')
ylabel('mean R/R0 BAG')
