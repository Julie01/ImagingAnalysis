...
%     eq_ratio_norm=smoothn(eq_ratio,20);
%     eq_ratio_norm=medfilt1(eq_ratio,20);
%     eq_ratio_norm(nanIdx)=NaN;
...
%     eq_ratio_norm_AFD=smoothn((eq_ratio_cent/nanstd(eq_ratio_cent)),10);
    ...
    SensNorm=sensIn-nanmedian(sensIn);
      SensNorm=smoothn(SensNorm./nanstd(SensNorm),30);
...
% set(gca,'XTickLabel',round(mDC(1,1:2:end)*10000)/10);
% title(' dRdT')
% xlabel(' CO2 change (‰/frame)')
% 
% subtightplot(1,2,2)
% bar(mBAG(1,:))
% set(gca,'XTick',[1:2:length(mDC(1,:))])
