...
for F=1:length(ImagingData)
   ...
   if norm==1
    ...
    % randomly sample same length episodes from Track:
    LE=ei-si(1:length(ei));
    rand_si=sort(randsample(1:length(fDF),length(LE)));
    rand_ei=rand_si+LE;
    
    oi=find(rand_ei>length(fDF));
    rand_ei(oi)=length(fDF);
 
    
    %cumsum of time or dF for these episodes:

    pVcs=NaN(1,length(fDF));
    pTcs=NaN(1,length(diffDF));
    for i=1:length(ei)
        pVcs(rand_si(i):rand_ei(i))=cumsum(fDF(rand_si(i):rand_ei(i)));
        pTcs(rand_si(i):rand_ei(i))=cumsum(ones(1,length((rand_si(i):rand_ei(i)))));
    end
