...
for F=1:length(ImagingData)
    ...
    for rev=1:length(RevEND)
        ...
        if nanmean(bearing(100:end))>95
            ...
            if RevON(rev)>600 & RevEND(rev)<length(ratio_norm)
                
                prerevUG(cc1,:)=NaN(1,960);
                revratio=ratio_norm(RevON(rev)-599:RevEND(rev));
                prerevUG(cc1,1:length(revratio))=ratio_norm(RevON(rev)-599:RevEND(rev));
                cc1=cc1+1;
                % if reversal is too close to start of trace:
            elseif RevON(rev)<600
                
                prerevUG(cc1,:)=NaN(1,960);
                revratio=ratio_norm(1:RevEND(rev));
               prerevUG(cc1,601-(RevON(rev)):600-(RevON(rev))+length(revratio))=revratio;
                cc1=cc1+1;
                %or to end
            elseif RevON(rev)+600>length(ratio_norm)
                prerevUG(cc1,:)=NaN(1,960);
                missing=length(ratio_norm)-(RevON(rev)+600);
                prerevUG(cc1,1:960+missing)=ratio_norm(RevON(rev)-599:end);
                cc1=cc1+1;
            end
        ...
        elseif nanmean(bearing(100:end))<85
            ...
            if RevON(rev)>600 & RevEND(rev)<length(ratio_norm)
                ...
                revratio=ratio_norm(RevON(rev)-599:RevEND(rev));
                prerevDG(cc2,1:length(revratio))=ratio_norm(RevON(rev)-599:RevEND(rev));
                cc2=cc2+1;
                % if reversal is too close to start of trace:
            elseif RevON(rev)<600
            ...
            elseif RevON(rev)+600>length(ratio_norm)
                prerevDG(cc2,:)=NaN(1,960);
                missing=length(ratio_norm)-(RevON(rev)+600);
                prerevDG(cc2,1:960+missing)=ratio_norm(RevON(rev)-599:end);
                cc2=cc2+1;
            end
...
h1(1)=shadedErrorBar2(-20:0.0333334:12,nanmean(prerevUG),sem,'b',0);
hold on
...
h1(2)=shadedErrorBar2(-20:0.0333334:12,nanmean(prerevDG),sem,'b',0);
hold on
